import { _decorator, Component, find, Node, tween, UIOpacity, UITransform, v3 } from "cc";
...
export class DestiPanel extends Component {
    ...
    onLoad() {
        this.bottomNode = find("Bottom", this.node);
        this.topNode = find("Top", this.node);
        const topNodeColor = SkinsManager.getInstance().getTopNodeColor();
        const bottomNodeColor = SkinsManager.getInstance().getBottomNodeColor();
        Tool.setNodeGradient(this.topNode, topNodeColor);
        Tool.setNodeGradient(this.bottomNode, bottomNodeColor);
        ...
        this.bottomNode.scale = v3(1, 0.8, 1);
        this.topNode.scale = v3(1, 0.8, 1);
        const bottomOpacity = this.bottomNode.getComponent(UIOpacity) ?? this.bottomNode.addComponent(UIOpacity);
        const topOpacity = this.topNode.getComponent(UIOpacity) ?? this.topNode.addComponent(UIOpacity);
        bottomOpacity.opacity = 0;
    ...
    playPhase0Ani() {
        const bottomOpacity = this.bottomNode.getComponent(UIOpacity);
        ...
        tween(this.compGemDestiUIOfPanel.getComponent(UIOpacity))
            ...
            .start();
        tween(this.bottomNode)
            .to(0.3, { scale: v3(1, 1, 1) }, { easing: "backOut" })
            .start();
        tween(bottomOpacity).to(0.3, { opacity: 255 }, { easing: "backOut" }).start();
        tween(this.topNode)
            .to(0.3, { scale: v3(1, 1, 1) }, { easing: "backOut" })
            .start();
    ...
    playPhase1Ani() {
        ...
        for (let i = destiDatas.length - 1; i >= 0; i--) {
            Logger.info("DestiPanel:playPhase1Ani", "destiDatas[i].blockIdx:", destiDatas[i].blockIdx);
